...
/**
 * [GRAPH/HISTOGRAM][name SAM file for left read], 
 * [name SAM file for right read],
 * [min edge wight for accept], [dist between pair read], [contig len]
 * if first key word "HISTOGRAM" only one library and 
 * [name SAM file for left read], [name SAM file for right read],
...
int main(int argc, char **argv) {
    ...
    if (argv[1][0] == 'G') {
        ...
        while (pos < argc) {
            ...
            prg.read_and_filter_reads(argv[pos], argv[pos + 1],
                                      atoi(argv[pos + 3]));
